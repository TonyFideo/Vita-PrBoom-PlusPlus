...
/* Per-display-frame diagnostics for the legacy immediate pipeline. These are
 * intentionally kept internal to this vendored VitaGL build. */
extern uint32_t vgl_diag_frame_serial;
extern uint32_t vgl_diag_frame_draws;
extern uint32_t vgl_diag_frame_vertices;
extern uint32_t vgl_diag_frame_legacy_bytes;
extern uint32_t vgl_diag_frame_legacy_peak;
extern uint32_t vgl_diag_frame_legacy_overflows;
extern uint32_t vgl_diag_frame_dropped_draws;
extern uint32_t vgl_diag_frame_gxm_errors;
extern uint32_t vgl_diag_frame_last_gxm_error;
extern uint32_t vgl_diag_frame_stream_errors;
extern uint32_t vgl_diag_frame_last_stream_error;
extern uint32_t vgl_diag_frame_circular_requested;
extern uint32_t vgl_diag_frame_circular_peak;
extern uint32_t vgl_diag_frame_circular_spills;
extern uint32_t vgl_diag_frame_circular_spill_bytes;
extern uint32_t vgl_diag_frame_alloc_failures;
extern uint32_t vgl_diag_frame_scene_resets;
extern uintptr_t vgl_diag_legacy_pool_base;
extern uint32_t vgl_diag_legacy_pool_size;
extern uint32_t vgl_diag_legacy_pool_global_peak;
extern GLboolean vgl_diag_ffp_failed;
#endif

/* True only while sceGxm has accepted the current scene.  A failed
 * BeginScene must make subsequent GL calls no-ops; sending streams or draws
 * to a rejected scene is unsafe on Vita and can become an access violation
 * in Vita3K. */
extern GLboolean vgl_gxm_scene_active;
