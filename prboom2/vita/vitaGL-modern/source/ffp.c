...
static GLboolean vgl_diag_current_draw_overflow = GL_FALSE;
static uint32_t vgl_diag_current_draw_stride = 0;
static uint32_t vgl_diag_current_draw_written = 0;
GLboolean vgl_diag_ffp_failed = GL_FALSE;

/* These fixed-function program objects are defined below in this file. */
extern SceGxmProgram *ffp_vertex_program;
extern SceGxmProgram *ffp_fragment_program;
extern SceGxmVertexProgram *ffp_vertex_program_patched;
extern SceGxmFragmentProgram *ffp_fragment_program_patched;

static inline uint32_t vgl_diag_legacy_stride(void) {
	if (texture_units[1].state)
		return LEGACY_MT_VERTEX_STRIDE;
	if (texture_units[0].state)
		return LEGACY_VERTEX_STRIDE;
	return LEGACY_NT_VERTEX_STRIDE;
}

static inline void vgl_diag_update_legacy_peak(void) {
	if (vgl_diag_legacy_pool_base && legacy_pool_ptr &&
		(uintptr_t)legacy_pool_ptr >= vgl_diag_legacy_pool_base) {
		uint32_t used = (uint32_t)((uintptr_t)legacy_pool_ptr - vgl_diag_legacy_pool_base);
		if (used > vgl_diag_frame_legacy_peak)
			vgl_diag_frame_legacy_peak = used;
		if (used > vgl_diag_legacy_pool_global_peak)
			vgl_diag_legacy_pool_global_peak = used;
	}
}

static inline void vgl_diag_mark_legacy_overflow(uint32_t stride) {
	if (!vgl_diag_current_draw_overflow) {
		vgl_diag_current_draw_overflow = GL_TRUE;
		vgl_diag_frame_legacy_overflows++;
		vgl_log("[VGL-DIAG] legacy vertex write blocked: frame=%u used=%u pool=%u stride=%u vertex=%u\n",
			vgl_diag_frame_serial,
			(vgl_diag_legacy_pool_base && legacy_pool_ptr &&
			 (uintptr_t)legacy_pool_ptr >= vgl_diag_legacy_pool_base) ?
			 (uint32_t)((uintptr_t)legacy_pool_ptr - vgl_diag_legacy_pool_base) : 0,
			vgl_diag_legacy_pool_size, stride, vertex_count);
	}
}

static inline void vgl_diag_mark_ffp_failure(const char *stage) {
	vgl_diag_ffp_failed = GL_TRUE;
	vgl_log("[VGL-DIAG] FFP failure: stage=%s frame=%u context=0x%08X vertex=0x%08X fragment=0x%08X vertex_patched=0x%08X fragment_patched=0x%08X\n",
		stage, vgl_diag_frame_serial,
		(uint32_t)(uintptr_t)gxm_context,
		(uint32_t)(uintptr_t)ffp_vertex_program,
		(uint32_t)(uintptr_t)ffp_fragment_program,
		(uint32_t)(uintptr_t)ffp_vertex_program_patched,
		(uint32_t)(uintptr_t)ffp_fragment_program_patched);
}

static inline GLboolean vgl_diag_ffp_ready(const char *stage) {
	if (!vgl_diag_ffp_failed)
		return GL_TRUE;
	vgl_diag_frame_dropped_draws++;
	vgl_log("[VGL-DIAG] draw skipped after FFP failure: stage=%s frame=%u\n",
		stage, vgl_diag_frame_serial);
	return GL_FALSE;
}

static inline void vgl_diag_immediate_stage(const char *stage) {
	if (vgl_diag_frame_draws <= 8) {
		vgl_log("[VGL-DIAG] immediate frame=%u draw=%u stage=%s context=0x%08X pool=0x%08X ptr=0x%08X vertices=%u\n",
			vgl_diag_frame_serial, vgl_diag_frame_draws, stage,
			(uint32_t)(uintptr_t)gxm_context,
			(uint32_t)(uintptr_t)legacy_pool,
			(uint32_t)(uintptr_t)legacy_pool_ptr, vertex_count);
	}
}
#endif

/* A failed texture upload used to leave a zeroed SceGxmTexture in the slot.
 * Letting a draw use that descriptor can turn a recoverable allocation
 * failure into a Vita/Vita3K access violation. */
static inline GLboolean vgl_texture_ready(const texture *tex, int unit, const char *stage) {
	if (tex && tex->status == TEX_VALID)
		return GL_TRUE;
#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_frame_alloc_failures++;
	vgl_diag_frame_dropped_draws++;
	vgl_log("[VGL-DIAG] draw skipped for invalid texture: stage=%s unit=%d texture=0x%08X status=%d data=0x%08X palette=0x%08X\n",
		stage, unit, (uint32_t)(uintptr_t)tex,
		tex ? tex->status : TEX_UNUSED,
		tex ? (uint32_t)(uintptr_t)tex->data : 0,
		tex ? (uint32_t)(uintptr_t)tex->palette_data : 0);
#endif
	return GL_FALSE;
}
...
uint8_t reload_ffp_shaders(SceGxmVertexAttribute *attrs, SceGxmVertexStream *streams, SceGxmIndexSource index_type) {
	...
	if (ffp_dirty_vert) {
		...
		if (f >= 0) {
			...
			if (!ffp_vertex_program) {
				sceIoClose(f);
#ifdef VITA_GL_DIAGNOSTICS
				vgl_diag_mark_ffp_failure("vertex cache allocation");
				return draw_mask_state;
#endif
			}
			int read_size = sceIoRead(f, ffp_vertex_program, size);
			sceIoClose(f);
#ifdef VITA_GL_DIAGNOSTICS
			if (read_size != (int)size) {
				vgl_diag_mark_ffp_failure("vertex cache read");
				return draw_mask_state;
			}
#endif
		} else {
			...
			#ifdef VITA_GL_DIAGNOSTICS
			if (!t || !size) {
				vgl_diag_mark_ffp_failure("vertex shader compile");
				shark_clear_output();
				return draw_mask_state;
			}
			#endif
			...
			#ifdef VITA_GL_DIAGNOSTICS
			if (!ffp_vertex_program) {
				vgl_diag_mark_ffp_failure("vertex program allocation");
				shark_clear_output();
				return draw_mask_state;
			}
			#endif
		...
		int vertex_register_result = sceGxmShaderPatcherRegisterProgram(gxm_shader_patcher, ffp_vertex_program, &ffp_vertex_program_id);
#ifdef VITA_GL_DIAGNOSTICS
		if (vertex_register_result) {
			vgl_diag_mark_ffp_failure("vertex shader registration");
			return draw_mask_state;
		}
#endif
		...
		#ifdef VITA_GL_DIAGNOSTICS
		if (ffp_vertex_unif_buf_size && !ffp_vertex_unif_buf) {
			vgl_diag_mark_ffp_failure("vertex uniform allocation");
			return draw_mask_state;
		}
		#endif
	...
	if (ffp_dirty_vert_attr) {
		...
		#ifdef VITA_GL_DIAGNOSTICS
		ffp_vertex_program_patched = NULL;
		#endif
		...
		#ifdef VITA_GL_DIAGNOSTICS
		if (!ffp_vertex_program_patched) {
			vgl_diag_mark_ffp_failure("vertex shader patch");
			return draw_mask_state;
		}
		#endif
	...
	if (ffp_dirty_frag) {
		...
		if (f >= 0) {
			...
			if (!ffp_fragment_program) {
				sceIoClose(f);
#ifdef VITA_GL_DIAGNOSTICS
				vgl_diag_mark_ffp_failure("fragment cache allocation");
				return draw_mask_state;
#endif
			}
			int read_size = sceIoRead(f, ffp_fragment_program, size);
			sceIoClose(f);
#ifdef VITA_GL_DIAGNOSTICS
			if (read_size != (int)size) {
				vgl_diag_mark_ffp_failure("fragment cache read");
				return draw_mask_state;
			}
#endif
		} else {
			...
			#ifdef VITA_GL_DIAGNOSTICS
			if (!t || !size) {
				vgl_diag_mark_ffp_failure("fragment shader compile");
				shark_clear_output();
				return draw_mask_state;
			}
			#endif
			...
			#ifdef VITA_GL_DIAGNOSTICS
			if (!ffp_fragment_program) {
				vgl_diag_mark_ffp_failure("fragment program allocation");
				shark_clear_output();
				return draw_mask_state;
			}
			#endif
		...
		int fragment_register_result = sceGxmShaderPatcherRegisterProgram(gxm_shader_patcher, ffp_fragment_program, &ffp_fragment_program_id);
#ifdef VITA_GL_DIAGNOSTICS
		if (fragment_register_result) {
			vgl_diag_mark_ffp_failure("fragment shader registration");
			return draw_mask_state;
		}
#endif
		...
		#ifdef VITA_GL_DIAGNOSTICS
		if (ffp_fragment_unif_buf_size && !ffp_fragment_unif_buf) {
			vgl_diag_mark_ffp_failure("fragment uniform allocation");
			return draw_mask_state;
		}
		#endif
	...
	#ifdef VITA_GL_DIAGNOSTICS
		ffp_fragment_program_patched = NULL;
	#endif
	...
	#ifdef VITA_GL_DIAGNOSTICS
		if (!ffp_fragment_program_patched) {
			vgl_diag_mark_ffp_failure("fragment shader patch");
			return draw_mask_state;
		}
	#endif
...
#ifdef VITA_GL_DIAGNOSTICS
	if (!gxm_context || !ffp_vertex_program_patched || !ffp_fragment_program_patched) {
		vgl_diag_mark_ffp_failure("FFP state before bind");
		return draw_mask_state;
	}
#endif
	...
	if (dirty_frag_unifs) {
		if (ffp_fragment_unif_buf_size) {
			...
			if (!buffer) {
#ifdef VITA_GL_DIAGNOSTICS
				vgl_diag_mark_ffp_failure("fragment uniform buffer reservation");
#endif
				return draw_mask_state;
			}
	...
	if (dirty_vert_unifs) {
		...
		if (!buffer && ffp_vertex_unif_buf_size) {
#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_mark_ffp_failure("vertex uniform buffer reservation");
#endif
			return draw_mask_state;
		}
...
void _glDrawArrays_FixedFunctionIMPL(GLint first, GLsizei count) {
...
#ifdef VITA_GL_DIAGNOSTICS
	if (!vgl_diag_ffp_ready("glDrawArrays"))
		return;
#endif
#ifdef HAVE_PROFILING
	...
	for (int i = 0; i < ffp_mask.num_textures; i++) {
		...
		if (!vgl_texture_ready(tex, i, "glDrawArrays")) {
...
void _glMultiDrawArrays_FixedFunctionIMPL(SceGxmPrimitiveType gxm_p, uint16_t *idx_ptr, const GLint *first, const GLsizei *count, GLint lowest, GLsizei highest, GLsizei drawcount) {
	uint8_t mask_state = reload_ffp_shaders(NULL, NULL, SCE_GXM_INDEX_SOURCE_INDEX_16BIT);
#ifdef VITA_GL_DIAGNOSTICS
	if (!vgl_diag_ffp_ready("glMultiDrawArrays"))
		return;
#endif
#ifdef HAVE_PROFILING
	uint32_t draw_start = sceKernelGetProcessTimeLow();
	...
	for (int i = 0; i < ffp_mask.num_textures; i++) {
		texture *tex = &texture_slots[texture_units[base_texture_id + i].tex_id[texture_units[base_texture_id + i].state > 1 ? 0 : 1]];
		if (!vgl_texture_ready(tex, i, "glMultiDrawArrays")) {
#ifndef SKIP_ERROR_HANDLING
			SET_GL_ERROR(GL_INVALID_OPERATION)
#endif
			return;
...
void _glDrawElements_FixedFunctionIMPL(uint16_t *idx_buf, GLsizei count, uint32_t top_idx, uint32_t base_idx, SceGxmIndexSource index_type) {
...
#ifdef VITA_GL_DIAGNOSTICS
	if (!vgl_diag_ffp_ready("glDrawElements"))
		return;
#endif
#ifdef HAVE_PROFILING
	uint32_t draw_start = sceKernelGetProcessTimeLow();
	...
	for (int i = 0; i < ffp_mask.num_textures; i++) {
		texture *tex = &texture_slots[texture_units[base_texture_id + i].tex_id[texture_units[base_texture_id + i].state > 1 ? 0 : 1]];
		if (!vgl_texture_ready(tex, i, "glDrawElements")) {
#ifndef SKIP_ERROR_HANDLING
			SET_GL_ERROR(GL_INVALID_OPERATION)
#endif
			return;
...
inline void glVertex3f(GLfloat x, GLfloat y, GLfloat z) {
	...
	if (_vgl_enqueue_list_func(glVertex3f, DLIST_FUNC_F32_F32_F32, x, y, z))
		return;
#endif
	/* A rejected BeginScene leaves the legacy pool unavailable. */
	if (!vgl_gxm_scene_active)
		return;
#ifndef SKIP_ERROR_HANDLING
	// Error handling
...
#ifdef VITA_GL_DIAGNOSTICS
	{
		uint32_t diag_stride = vgl_diag_legacy_stride();
		if (!legacy_pool_ptr) {
			vgl_diag_mark_legacy_overflow(diag_stride);
			vertex_count++;
			return;
		}
#ifndef SKIP_ERROR_HANDLING
		if (!legacy_pool_end ||
			(uintptr_t)legacy_pool_ptr + (uintptr_t)diag_stride * sizeof(float) >
			(uintptr_t)legacy_pool_end) {
			vgl_diag_mark_legacy_overflow(diag_stride);
			vertex_count++;
			return;
		}
#endif
		vgl_diag_current_draw_stride = diag_stride;
	}
#endif
	...
	vertex_count++;
#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_frame_vertices++;
	vgl_diag_frame_legacy_bytes += vgl_diag_current_draw_stride * sizeof(float);
	vgl_diag_current_draw_written++;
	vgl_diag_update_legacy_peak();
#endif
}
...
void glBegin(GLenum mode) {
	...
	vertex_count = 0;
#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_current_draw_overflow = GL_FALSE;
	vgl_diag_current_draw_stride = 0;
	vgl_diag_current_draw_written = 0;
#endif
}
...
void glEnd(void) {
...
#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_frame_draws++;
	vgl_diag_immediate_stage("begin");
	if (vgl_diag_current_draw_overflow) {
		vgl_diag_frame_dropped_draws++;
		if (vgl_diag_frame_dropped_draws <= 8) {
			vgl_log("[VGL-DIAG] immediate draw dropped: frame=%u vertices=%u written=%u stride=%u\n",
				vgl_diag_frame_serial, vertex_count, vgl_diag_current_draw_written,
				vgl_diag_current_draw_stride);
		}
		/* No vertex from this draw was submitted. Reuse its start address
		 * rather than advancing into memory beyond the pool. */
	...
	if (texture_units[1].state) { // Multitexture usage
		...
		#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_immediate_stage("ffp_reload_multitexture_begin");
		#endif
		reload_ffp_shaders(legacy_mt_vertex_attrib_config, legacy_mt_vertex_stream_config, SCE_GXM_INDEX_SOURCE_INDEX_16BIT);
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_immediate_stage("ffp_reload_multitexture_done");
		if (!vgl_diag_ffp_ready("glEnd multitexture")) {
		...
		for (int i = 0; i < 2; i++) {
			...
			if (!vgl_texture_ready(tex, i, "glEnd multitexture")) {
				ffp_vertex_attrib_state = orig_state;
				legacy_pool_ptr = legacy_pool;
				restore_polygon_mode(prim);
				return;
			...
			#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_immediate_stage("texture_bind_before");
			#endif
			sceGxmSetFragmentTexture(gxm_context, i, &tex->gxm_tex);
			#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_immediate_stage("texture_bind_done");
			#endif
		}
	...
	#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_immediate_stage("ffp_reload_texturing_begin");
	#endif
		reload_ffp_shaders(legacy_vertex_attrib_config, legacy_vertex_stream_config, SCE_GXM_INDEX_SOURCE_INDEX_16BIT);
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_immediate_stage("ffp_reload_texturing_done");
		if (!vgl_diag_ffp_ready("glEnd texturing")) {
			ffp_vertex_attrib_state = orig_state;
			legacy_pool_ptr = legacy_pool;
			restore_polygon_mode(prim);
			return;
		...
		if (!vgl_texture_ready(tex, 0, "glEnd texturing")) {
			ffp_vertex_attrib_state = orig_state;
			legacy_pool_ptr = legacy_pool;
			restore_polygon_mode(prim);
			return;
		...
		#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_immediate_stage("ffp_reload_untextured_begin");
		#endif
		reload_ffp_shaders(legacy_nt_vertex_attrib_config, legacy_nt_vertex_stream_config, SCE_GXM_INDEX_SOURCE_INDEX_16BIT);
	#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_immediate_stage("ffp_reload_untextured_done");
		if (!vgl_diag_ffp_ready("glEnd untextured")) {
			ffp_vertex_attrib_state = orig_state;
			legacy_pool_ptr = legacy_pool;
			restore_polygon_mode(prim);
			return;
	...
	if (vgl_diag_ffp_failed) {
		vgl_diag_frame_dropped_draws++;
		if (vgl_diag_frame_dropped_draws <= 8) {
			vgl_log("[VGL-DIAG] immediate draw dropped after FFP failure: frame=%u vertices=%u\n",
				vgl_diag_frame_serial, vertex_count);
		}
		legacy_pool_ptr = legacy_pool;
		restore_polygon_mode(prim);
		return;
	...
	if (!gxm_context || !legacy_pool || !vertex_count) {
#ifdef VITA_GL_DIAGNOSTICS
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] immediate draw skipped before stream setup: context=0x%08X vertex_pool=0x%08X vertices=%u\n",
			(uint32_t)(uintptr_t)gxm_context,
			(uint32_t)(uintptr_t)legacy_pool, vertex_count);
#endif
		legacy_pool_ptr = legacy_pool;
		restore_polygon_mode(prim);
		return;
	...
	#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_immediate_stage("stream_setup_before");
	#endif
	...
	#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_immediate_stage("stream_setup_done");
	#endif
...
#ifdef VITA_GL_DIAGNOSTICS
			vgl_diag_frame_alloc_failures++;
			vgl_log("[VGL-DIAG] line-loop index allocation failed: bytes=%u\n",
				vertex_count * 2 * (uint32_t)sizeof(uint16_t));
#endif
			restore_polygon_mode(prim);
			return;
		}
...
#ifdef VITA_GL_DIAGNOSTICS
	if (!gxm_context || !legacy_pool || !ptr || !index_count) {
		vgl_diag_frame_alloc_failures++;
		vgl_log("[VGL-DIAG] immediate draw skipped: context=0x%08X vertex_pool=0x%08X index=0x%08X indices=%u\n",
			(uint32_t)(uintptr_t)gxm_context,
			(uint32_t)(uintptr_t)legacy_pool,
			(uint32_t)(uintptr_t)ptr, index_count);
		legacy_pool_ptr = legacy_pool;
		restore_polygon_mode(prim);
		return;
	}
	...
	#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_immediate_stage("draw_before");
	#endif
	int draw_result = sceGxmDraw(gxm_context, prim, SCE_GXM_INDEX_FORMAT_U16, ptr, index_count);
	#ifdef VITA_GL_DIAGNOSTICS
	vgl_diag_immediate_stage("draw_done");
	#endif
#ifdef VITA_GL_DIAGNOSTICS
	if (draw_result) {
		vgl_diag_frame_gxm_errors++;
		vgl_diag_frame_last_gxm_error = (uint32_t)draw_result;
		vgl_log("[VGL-DIAG] sceGxmDraw failed: primitive=%d vertices=%u indices=%u result=0x%08X (%s)\n",
			(int)prim, vertex_count, index_count, (uint32_t)draw_result,
			get_gxm_error_literal(draw_result));
	}
#endif
