      ...
      if (!--door->topcountdown)  // downcount and check
      {
        switch(door->type)
        {
          ...
          case genBlazeRaise:
            ...
            S_StartSoundAtOrigin(&door->sector->soundorg,sfx_bdcls);
            break;

          case normal:
          case genRaise:
            door->direction = -1; // time to go back down
            S_StartSoundAtOrigin(&door->sector->soundorg,sfx_dorcls);
            break;

          case close30ThenOpen:
          case genCdO:
            door->direction = 1;  // time to go back up
            S_StartSoundAtOrigin(&door->sector->soundorg,sfx_doropn);
            break;

          case genBlazeCdO:
            door->direction = 1;  // time to go back up
            S_StartSoundAtOrigin(&door->sector->soundorg,sfx_bdopn);
            break;
  ...
  mobj_t* thing )
{
  ...
  switch(line->special)
  {
    ...
    case 118: // blazing door open
      S_StartSoundAtOrigin(&sec->soundorg,sfx_bdopn);
      break;

    default:  // normal or locked door sound
      S_StartSoundAtOrigin(&sec->soundorg,sfx_doropn);
      break;
  }
